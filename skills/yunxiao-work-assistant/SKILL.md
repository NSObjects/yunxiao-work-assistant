...
4. 估算与排期：
   ...
   - 默认每天按 `8h` 常规工作容量排期；当天最多追加 `2-3h` 加班容量。加班不是默认容量，只有当前迭代或延期旧迭代事项按常规容量无法完成、且本周确有明确交付风险时才安排。
   - 容量计算先用“剩余工作日 × 8h”得出常规容量，再预留 10%-20% 机动时间；如需加班，单独列出加班日期、小时数和原因。不要把加班时间也排满，仍需保留必要机动。
   ...
   - `本周分配时间` 要写到具体日期和时间段，例如 `周二全天 8h，周三上午 4h`；使用加班时标注为 `周三晚上加班 2h`。不排入本周的事项写 `本周不排`，并说明原因。
   - 同一需求或大任务要尽量连续安排为半天、全天或相邻多天时间块；开始后不要在多个需求之间频繁来回切换。除线上问题、测试阻塞、发布窗口、外部联调窗口外，不要把同一需求拆成零散的 1-2h 碎片。
   - 排期时先估算每项剩余耗时，再按优先级和连续性放入日程；一个需求超过单日可用容量时，优先连续安排到相邻工作日。超过本周常规容量但可通过少量加班完成时，可以安排加班，并说明为什么需要加班。
   - 如果工作项超过本周“常规容量 + 必要加班容量”，只安排本周可交付目标，剩余部分放入候补或下周延续，并明确不足小时数；不要为了排满清单把大需求硬塞进本周。
   - 默认按剩余工作日做容量规划；没有可靠节假日信息时按普通工作日估算并说明假设。排期总量不要超过可用容量，至少预留 10%-20% 机动时间处理测试反馈、线上问题和会议。
   - 优先把测试中、开发完成、发布生产、阻塞项和线上问题安排在前面；大块新开发如果超过本周剩余容量，应放入候补或拆成本周可交付目标，而不是硬塞进计划。
5. 排序规则：
   - 阻塞项、线上问题、最高优先级事项优先。
   - 已在进行中的事项优先于还未启动的事项。
   - 能在本周形成明确交付的事项优先。
...
5. 关联云效工作项与代码提交：
   ...
   - 无法确认归属的提交不单独扩展周报区块，只有能说明本周完成情况时才纳入对应内容。
6. 输出格式：
   - `本周工作完成情况`
   - `下周计划`
7. 周报写法要求：
