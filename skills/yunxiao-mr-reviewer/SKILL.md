...
- 云效 MR 评论按“评论排版规范”里的通用 Markdown 模板编写，优先使用标题、短列表和短代码块；不要使用 Mermaid、HTML、表格或依赖特定渲染组件的复杂格式。
...
- 不使用 Markdown 表格、HTML、Mermaid、脚注、折叠块、连续多级嵌套列表或装饰性分隔线。
...
4. 调用 `create_change_request_comment`：
   ...
   - 评论正文必须套用“评论排版规范”的模板；行内评论优先用短列表，全局总结使用固定二级/三级标题、列表和普通文本流程说明，不使用 Mermaid。
