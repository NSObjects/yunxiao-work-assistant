...
- **已读取**：列出 `AGENTS.md` / `AGENT.md` 路径；没有则写“未发现项目 AGENT 指南”。
- **影响本次审核的约定**：列出和本 MR 直接相关的约定。

### 3. 规格对应关系

- **已读取**：列出 `spec/` 文件、工作项或 MR 描述来源。
- **覆盖情况**：说明已覆盖的需求点。
- **未覆盖/未确认**：说明缺失的 spec、验收点或业务确认项。
...
- **潜在风险**：只写由 diff、spec 或现有证据支持的风险；没有则写“未发现明显潜在风险”。
...
- **已审查材料**：列出 MR 描述、diff、提交、`AGENTS.md` / `AGENT.md`、`spec/`、工作项、已有评论等实际读取的材料。
- **未审查/受限**：列出权限不足、文件过大、未取得 spec、无法读取旧版本等限制；没有则写“未发现受限材料”。
